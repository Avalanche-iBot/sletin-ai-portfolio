...
/**
 * Case study 01 — AI patient communication platform. The flagship.
 *
 * The reference note for the whole site: the longest, the most complete, and
 * the one whose voice the others are measured against.
 *
 * Its architectural position is that this is a routing problem rather than a
 * model problem. Roughly three quarters of inbound volume turns out to be
 * deterministically answerable, and that share — not the choice of model — is
 * what decides whether the system is affordable. So classification and
 * deterministic skills sit in front of the model, and every path that reaches
 * the model has to justify itself.
 *
 * The scenario is constructed. Figures are assumptions chosen so the
 * constraints bind; where a number carries architectural weight, the section
 * using it shows the derivation and says what changes if it is wrong.
 */
const caseStudy: CaseStudy = {
  ...
  duration: "Assumed programme length: 6 months to production",
  githubUrl: "",
  liveDemoUrl: "",
  ...
  impact: "Target: 12 min → <30 s response time · ≥70% requests automated · ~€0.006 blended AI cost per message",
  tags: [
    "RAG",
    "Hybrid routing",
    "GDPR",
    ...
    model: [
      {
        ...
        v: "(input tokens x input price) + (output tokens x output price) + one retrieval query"
      },
      {
  ...
  kpis: [
    ...
    },
    {
      ...
      kpi: "End-to-end response time, 95th percentile (p95)",
      baseline: "No baseline",
      target: "< 3 sec",
      why: "The 95th percentile rather than the average: the average is dominated by cache hits and would look excellent while one patient in twenty waits."
    },
    {
      category: "Latency & reliability",
      kpi: "Deterministic-path response time (p95)",
      baseline: "No baseline",
      target: "< 500 ms",
    ...
    diagrams: [
      {
        ...
        caption: "Every component the design touches, by tier. Deliberately unremarkable infrastructure: the only tier carrying original thinking is orchestration, which is where the routing decision lives.",
        rows: [
          {
            label: "Channels",
            nodes: [
        ...
        lanes: [
          {
            ...
            note: "Runs once per document, off the user path — so its cost is amortised and its latency is irrelevant."
          },
          {
            steps: [
            ...
            note: "Runs on every free-text request and must return inside the latency budget."
          }
        ]
      },
      {
        ...
        caption: "One escalation, traced end to end. Escalation is designed, measured and reported as a KPI rather than handled as a failure — which is why the decision to escalate is a logged step rather than an exception.",
        actors: [
          "Patient",
          "Orchestrator",
          "AI layer",
