...
export default function ArchitecturePage() {
  return (
    <div>
      {/*
       * Header lives in its own full-width band rather than inside the shell
       * below, so `grid-field`'s background — position:absolute, inset:0
       * against its own box — spans the full viewport the way it does on the
       * homepage hero and every case note's header. Putting the class directly
       * on a `shell`-constrained element instead confines the pattern to the
       * shell's max-width column, which is a visibly narrower effect.
       */}
      <header className="grid-field relative overflow-hidden border-b border-line">
        <div className="shell max-w-reading py-16 sm:py-24">
      ...
      <div className="shell py-16 sm:py-24">
        {/*
         * Open, not a disclosure. The page carries three separate tools now,
         * and the two most useful sit a long way down — a reader who cannot
         * see that the builder exists will never scroll to find it.
         */}
        <nav aria-label="On this page" className="border-y border-line py-4">
